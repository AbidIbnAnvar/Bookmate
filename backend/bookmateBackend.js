...
app.post("/create-admin", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Admin created successfully!", details: admin });
  } catch (error) {
    return res.status(500).json({ message: "Error occurred when creating admin!", error });
  }
})
